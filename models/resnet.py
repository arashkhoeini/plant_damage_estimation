...
def load_url(url, model_dir='./pretrained', map_location=None):
    """
    Load model from URL with proper validation and error handling.
    
    Args:
        url (str): URL to download model from
        model_dir (str): Directory to save downloaded models
    ...
    Raises:
        ValueError: If URL is not from trusted sources
        RuntimeError: If download or extraction fails
    """
    # Validate URL is from trusted sources
    trusted_domains = ['download.pytorch.org', 's3.us-west-1.wasabisys.com']
    if not any(domain in url for domain in trusted_domains):
        raise ValueError(f"Untrusted URL domain: {url}")
    ...
    if not os.path.exists(cached_file):
        ...
        try:
            ...
            if os.path.exists(cached_file):
                os.remove(cached_file)
            raise RuntimeError(f"Failed to extract {cached_file}: {e}")
