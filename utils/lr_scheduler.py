...
class Poly(_LRScheduler):
    ...
    Implements polynomial learning rate decay with optional warmup period.
    The learning rate is computed as: base_lr * (1 - iter/max_iter)^power

    Args:
        optimizer (Optimizer): Wrapped optimizer
        num_epochs (int): Number of training epochs
        iters_per_epoch (int): Number of iterations per epoch (default: 0)
    ...
    def get_lr(self):
        """
        Compute learning rate based on polynomial decay schedule.
...
class OneCycle(_LRScheduler):
    ...
    Implements the one-cycle learning rate policy described in:
    ...
    Args:
        ...
        iters_per_epoch (int): Number of iterations per epoch (default: 0)
        last_epoch (int): The index of last epoch (default: -1)
        momentums (tuple): Min and max momentum values (default: (0.85, 0.95))
        div_factor (int): Determines initial learning rate (default: 25)
        phase1 (float): Fraction of cycle spent in first phase (default: 0.3)
    """
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.SGD(params=resnet.parameters(), **params)  # type: ignore
    ...
    for epoch in range(epochs):
        for i in range(iters_per_epoch):
            ...
            lr_scheduler(optimizer, i, epoch)  # type: ignore
